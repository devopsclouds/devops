...
spec:
  hard:
    pods: "2"
    configmaps: "2"
Above example to restrict the namespace myquota in that namespace it should allow 2 pods,2 configmaps to create it , u cannot create more then that 
...
Above example to restrict the namespace myquota in that namespace  500 Mi set the limits of memory and 100Mi set the requests of memory  u cannot use more then that limits and request memory of that namespace
...
spec:
  ...
  - image: nginx
    ...
    resources:
      ...
      requests:
        memory: "100Mi"
Here we are using limits 100 Mi and memory 100 Mi according to that resrict namespace
note . limits: the amount of memory should be use given that value the pod memory  cannot use more then that of limit memory
        requests: it will look specific workernode of that given request memory . minimum memory should be there then on that workernode

suppose if u dont want set the resources in pod then u can create limit-range of that namespace and it will take default limit and request of the pod
apiVersion: v1
kind: LimitRange
metadata:
  name: mem-limitrange
